...
summary.features.df <- list() # list of features

for (m in 1:rep.sample){
  ...
  seed.id <- 123 + m # seed for simulation
  ...
  # Due to much computation time of transmNetworkBuilder.diff3() we put it external to age-mixing function
  
  
  # 
  # trans.network <- transmNetworkBuilder.diff3(datalist = datalist, endpoint = datalist$itable$population.simtime[1])
  # 
  ...
  #### Combined all together  in a data frame ###
  
  
  summary.df <- c( m, # j iteration
  ...
  summary.features.df[[m]] <- summary.df
  
}



# Bind features for all runs

for (i in 1:rep.sample){
  if(i==1){
    summary.features.df.i.t <- summary.features.df[[i]]
  }
  else{
    
    red.summary.features.df.i <- summary.features.df[[i]]
    summary.features.df.i.t <- rbind(summary.features.df.i.t, red.summary.features.df.i)
  }
}
...
summary.features.df.t <- as.data.frame(summary.features.df.i.t)


# Name the columns

features.names <- c("sim.id",
                    
                    "concur.prev.6months.male", 
...
names(summary.features.df.t) <- c(features.names)


# Save features in CSV file
write.csv(summary.features.df.t, file = "summary.features.df.csv")
