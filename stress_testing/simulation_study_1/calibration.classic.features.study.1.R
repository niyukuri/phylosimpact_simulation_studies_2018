...
names.class <- names(read.csv("~/Desktop/mastermodeltest/names.classic.csv"))

names.phylo <- names(read.csv("~/Desktop/mastermodeltest/names.phylo.csv"))
...
simpact4ABC.classic <- function(inputvector){
  ...
  outputvector <- as.numeric(summary.stat) # names.class
...
sum_stat_obs <- c(0.003916345 , 0.000000000 , 0.000000000 , 0.000000000 , 
                  ...
                  1.033541863 , -1.386704665,  0.062130178
                  
                  # 6.003394756 , 0.285714286 , 0.588235294 , 
                  # 4.666666667 , 3.000000000 , 0.428571429 , 
                  # 1.142857143 , 1.000000000
)
