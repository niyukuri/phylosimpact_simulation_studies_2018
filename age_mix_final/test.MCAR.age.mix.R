    ...
    if (as.numeric(results["eventsexecuted"]) >= (as.numeric(cfg.list["population.maxevents"]) - 1)){
      ...
      if(length(big.index) >= 1){ 
        ...
        results.mcar <- c(summary.epidemic.rels.df, res.clust.MCAR.cov.35, res.clust.MCAR.cov.40, res.clust.MCAR.cov.45,
                          res.clust.MCAR.cov.50, res.clust.MCAR.cov.55, res.clust.MCAR.cov.60, res.clust.MCAR.cov.65,
                          res.clust.MCAR.cov.70, res.clust.MCAR.cov.75, res.clust.MCAR.cov.80, res.clust.MCAR.cov.85,
                          res.clust.MCAR.cov.90, res.clust.MCAR.cov.95)
        
        names(results.mcar) <- c(paste0(names(summary.epidemic.rels.df)),
                                 paste0("cov.",35,".",paste0(names(res.clust.MCAR.cov.35))), paste0("cov.",40,".",paste0(names(res.clust.MCAR.cov.40))),
                                 paste0("cov.",45,".",paste0(names(res.clust.MCAR.cov.45))), paste0("cov.",50,".",paste0(names(res.clust.MCAR.cov.50))),
                                 paste0("cov.",55,".",paste0(names(res.clust.MCAR.cov.55))), paste0("cov.",60,".",paste0(names(res.clust.MCAR.cov.60))),
                                 paste0("cov.",65,".",paste0(names(res.clust.MCAR.cov.65))), paste0("cov.",70,".",paste0(names(res.clust.MCAR.cov.70))),
                                 paste0("cov.",75,".",paste0(names(res.clust.MCAR.cov.75))), paste0("cov.",80,".",paste0(names(res.clust.MCAR.cov.75))),
                                 paste0("cov.",85,".",paste0(names(res.clust.MCAR.cov.80))), paste0("cov.",90,".",paste0(names(res.clust.MCAR.cov.85))),
                                 paste0("cov.",95,".",paste0(names(res.clust.MCAR.cov.90))))
...
# v <- test.MCAR.age.mix(inputvector = inputvector)
...
mcar.sim <- simpact.parallel(model = test.MCAR.age.mix,
                             actual.input.matrix = inputmatrix,
                             seed_count = 1000,
                             n_cluster = 8)

write.csv(mcar.sim, file = "Results.mcar.sim.csv")
